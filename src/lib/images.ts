...
export const getNewProduct2Images = unstable_cache(
    async () => {
        let allFiles: string[] = [];
        let page = 0;
        const limit = 100;
        ...
        while (hasMore) {
            ...
            if (error) {
                console.error("Error fetching new_products_2 images:", error);
                throw error;
            }
    ...
    ["new-products-2-images-list"],
    { revalidate: 60, tags: ["new_products_2"] }
);
